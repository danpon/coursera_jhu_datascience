#1
#Consider the space shuttle datashuttle in the MASS library. 
...
#as predicted by wind sign (variable wind). Give the estimated odds ratio for autolander 
#use comparing head winds, labeled as "head" in the variable headwind (numerator) to tail winds (denominator).
library(MASS)
data(shuttle)
...
#comparing head winds (numerator) to tail winds (denominator) adjusting for wind strength
...
# The coefficients reverse their signs. OK
#Remember that the coefficients are on the log scale.
#So changing the sign changes the numerator and denominator for the exponent.
...
#5
#Consider a Poisson glm with an offset, ttt. 
#So, for example, a model of the form glm(count ~ x + offset(t), family = poisson)
#where x is a factor variable comparing a treatment (1) to a control (0) and t
#is the natural log of a monitoring time. 
#What is impact of the coefficient for x if we fit the model glm(count ~ x + offset(t2), family = poisson) where 2 <- log(10) + t\verb|2 <- log(10) + t|2<-log(10)+t? 
...
#R) The coefficient estimate is divided by 10.
#R)The coefficient estimate is multiplied by 10.
#R) The coefficient is subtracted by log(10).
#R) The coefficient estimate is unchanged
